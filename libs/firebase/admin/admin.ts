...
import { moveCloudinaryImage } from "../../cloudinary/cloudinary";
...
export async function acceptUser(myId: string, userId: string, category: string) {
  try {
...
export async function getUsersPossibleAdmins(currentUserId: string) {
  try {
    const q = query(
      collection(db, "users"),
...
export async function setNewAdmin(myId: string, userId: string) {
  try {
    // Paso 1: Verificar que myId es admin
    const adminRef = doc(db, "users", myId);
    const adminSnap = await getDoc(adminRef);
